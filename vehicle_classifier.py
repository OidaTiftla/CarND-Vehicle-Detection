...
class VehicleClassifierTrainer:
    ...
    def train(self):
        ...
        for i in range(len(files_train)):
            ...
            # scaled_img = cv2.resize(img, (64+24, 64+24))
            # X_train.append(self.classifier.extract_features(scaled_img[0:64, 0:64]))
            # y_train.append(label)
            # X_train.append(self.classifier.extract_features(scaled_img[24:88, 0:64]))
            # y_train.append(label)
            # X_train.append(self.classifier.extract_features(scaled_img[0:64, 24:88]))
            # y_train.append(label)
            # X_train.append(self.classifier.extract_features(scaled_img[24:88, 24:88]))
            # y_train.append(label)
            # # augment training set
